...
- Findings, ranked:
  ...
  2. PARTIAL - Top-level wire-15 Autogrow families with static template inputs decode positional and keyed widget values after reconstructing every authored member. A nested dynamic family template emits `import.dynamic.autogrowUnsupported`, stops the node's positional walk, and preserves every remaining value in `ext["importer.excessWidgetValues"]`; it never guess-assigns later positions after alignment is lost.
  3. FIXED - Autogrow connections reconstruct schema-declared prefix and names vocabularies, including grouped forms, into persisted member state and resolvable member-qualified endpoints. Ordinary native prefix members mint `mN` identities in first input-slot appearance order; foreign numeric ordinals are recognition labels only, so sparse or out-of-order inputs never become sorting keys. Maintained alias snapshots preserve their source suffixes and authored order so family replacement can copy them one-for-one. Names families retain their declared names as identities. Unmatched family-scoped wire names emit `import.dynamic.autogrowWireUnknown` and remain unresolved for `solve.portMissing` rather than being guessed.
  4. HIGH - LiteGraph subgraph definitions are a hard stop (`import.subgraphs.unsupported`). Needs a separate importer design (definition/instance identity, boundary slots, widget values, ComfyUI's own documented dynamic-in-subgraph limitations).
...
- Remaining work: decide DynamicSlot foreign positional/connectivity semantics before decoding its dependents or treating it as an alignment boundary, and design subgraph import separately.
