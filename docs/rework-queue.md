...
- Findings, ranked:
  ...
  4. FIXED - LiteGraph subgraph definitions retain definition and instance identity, boundary slots, promoted widget values, nested structure, and save/reopen behavior. Boundaries unresolved against the current catalog retain their authored structure and report `import.subgraphs.boundaryUnresolved`; unrepresentable structural boundaries still use explicit per-instance fallback.
