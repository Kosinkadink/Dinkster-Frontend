...
The Library sidebar and starter gallery search template identity, names, descriptions, families, and tags. Search ignores case, spaces, punctuation, and common Stable Diffusion abbreviations: `SD 1.5`, `sd1.5`, and `SD15` match the same templates, as do `SDXL` and `Stable Diffusion XL`. The Library pack selector applies the backend's exact pack filter.
