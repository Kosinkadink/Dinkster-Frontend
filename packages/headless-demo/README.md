...
This private workspace package joins a shared Dinkster workflow and performs a
short sequence of visible graph edits. It copies two nodes and their link when
the workflow contains a usable node-to-node link. Otherwise, it copies one
node. The client marks the first copy and moves it in four steps so a browser
participant can watch the update.
...
```sh
pnpm --filter @dinkster/headless-demo start -- \
