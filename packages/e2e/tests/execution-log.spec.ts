...
test('execution log panel shows live records and changes mounted chrome locale', async ({ page, request }) => {
  ...
  await page.getByTestId('execution-log-node-select').selectOption(seed.lastNode)
  await expect(rows).toHaveCount(2)
  ...
  await expect(page.getByTestId('execution-log-node-select')).toHaveValue(seed.lastNode)
  await expect(rows).toHaveCount(2)
  await expect(rows.nth(0)).toContainText('denoising step batch complete')
  ...
  await expect(page.getByTestId('execution-log-run-select').locator('option').filter({ hasText: seed.execution.prompt }))
