 ...
 * (no webServer, no catalog mocks). Backend mutation routes are
 * intercepted defensively; the scenarios themselves never submit jobs
 * or upload assets.
 */
...
async function openFixture(page: Page): Promise<void> {
  ...
  await expect.poll(() => page.evaluate(() => {
    ...
    return tab !== undefined && 'status' in tab.store && tab.store.doc.lineage === 'audit-live-proof'
  })).toBe(true)
  await expect.poll(() => page.evaluate(() =>
    window.__dinksterTest!.renderer!.getScene().nodes.some((node) => node.id === 'tapSink'),
  )).toBe(true)
...
test.beforeEach(async ({ context, page, request }, testInfo) => {
...
test('live: connected and ordinary paste preserve endpoint rules', async ({ page }) => {
  for (const [sink, expected, screenshot] of [
    ['outputSink', { node: 'source', port: 'out' }, 'live-a-connected-output.png'],
    ['tapSink', { node: 'tap', tap: 'amount' }, 'live-b-connected-widget-tap.png'],
  ] as const) {
  ...
  await page.screenshot({ path: `${proofDir}/live-d-ordinary-paste-detached.png` })
})

test('live: stale widget-tap source is skipped while valid stubs and pasted nodes land', async ({ page }) => {
  ...
  await page.screenshot({ path: `${proofDir}/live-c-stale-tap-skipped-valid-stub-lands.png` })
})

test('live: output move-source empty drop disconnects, undo restores, Escape cancels, and a target moves', async ({ page }) => {
  ...
  await page.screenshot({ path: `${proofDir}/live-e-empty-drop-disconnected.png` })
  ...
  expect(state.links.some((link) => link.id === 'outputLink')).toBe(true)
  expect(state.revision).toBe(before.revision + 2)
  await page.screenshot({ path: `${proofDir}/live-f-escape-snapback.png` })
  ...
  await page.screenshot({ path: `${proofDir}/live-g-valid-target-moves-source.png` })
})

test('live: widget-tap and multi-link fan-out empty drops disconnect atomically', async ({ page }) => {
  ...
  await page.screenshot({ path: `${proofDir}/live-h-widget-tap-empty-drop.png` })
  await page.keyboard.press('Control+z')
  ...
  await page.screenshot({ path: `${proofDir}/live-i-fan-out-empty-drop.png` })
  await page.keyboard.press('Control+z')
  state = await graphState(page)
