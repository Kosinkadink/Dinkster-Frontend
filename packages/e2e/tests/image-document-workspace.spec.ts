...
const proofDir = (): string =>
  process.env['DINKSTER_IMAGE_DOCUMENT_PROOF_DIR'] ??
  evidenceGroupDir('issue-310')
...
test('edits and recovers a durable layered image document', async ({
  page,
}) => {
  ...
  const workflowTabId = await page.getByRole('tab').first().getAttribute('id')
  ...
  await workspace
    ...
    .setInputFiles(await rasterFile(page, 'coastline.png', 900, 560, 'base'))
  ...
  for (let step = 0; step < 7; step += 1) {
    await workspace
      .getByRole('slider', { name: 'Layer opacity' })
      .press('PageUp')
  }
  ...
  await workspace
    ...
    .setInputFiles(await rasterFile(page, 'fade mask.png', 420, 320, 'mask'))
  ...
  await workspace.screenshot({
    path: `${proofDir()}/image-document-workspace.png`,
    animations: 'disabled',
  })
  await page.locator(`#${workflowTabId}`).click()
  ...
  await page.reload()
  await page.locator('.tab[data-tab-id^="image:"] .tab-select').click()
  await expect(page.getByTestId('image-document-workspace')).toContainText(
    'color accent',
  )
  await expect(
    page
...
test('keeps workflow, image, and video documents in one tab strip', async ({
  page,
}, testInfo) => {
  await page.goto('/')
  const workflowTab = page.getByRole('tab').first()
  const workflowTabId = await workflowTab.getAttribute('id')
  const workflowDocumentId = await workflowTab.evaluate(
    (tab) => tab.closest('.tab')?.getAttribute('data-tab-id') ?? '',
  )

  await page.getByTestId('new-video-tab').click()
  const videoWorkspace = page.getByTestId('video-document-workspace')
  await expect(videoWorkspace).toBeVisible()
  const videoTab = page.locator('.tab[data-tab-id^="video:"] .tab-select')
  await expect(videoTab).toHaveCount(1)
  const videoJson = videoWorkspace.getByTestId('video-document-json')
  const authored = JSON.parse(await videoJson.inputValue())
  authored.settings.width = 1920
  await videoJson.fill(JSON.stringify(authored, null, 2))
  await videoWorkspace.getByRole('button', { name: 'Apply' }).click()
  await page.locator(`#${workflowTabId}`).click()
  await expect(videoWorkspace).toBeHidden()
  await videoTab.click()
  await expect(videoJson).toHaveValue(/1920/)

  const imageWorkspace = page.getByTestId('image-document-workspace')
  await imageWorkspace
    .locator('.image-document-tabs input[type=file]')
    .setInputFiles(await rasterFile(page, 'shared-tabs.png', 640, 360, 'base'))
  await expect(imageWorkspace).toBeVisible()
  await expect(
    page.locator('.tab[data-tab-id^="image:"] .tab-select'),
  ).toHaveCount(1)
  await expect(page.locator('.workflow-tablist [role="tab"]')).toHaveCount(3)
  await expect(page.locator('[data-document-kind="workflow"]')).toHaveCount(1)
  await expect(page.locator('[data-document-kind="image"]')).toHaveCount(1)
  await expect(page.locator('[data-document-kind="video"]')).toHaveCount(1)

  const imageTab = page.locator('.tab[data-tab-id^="image:"]')
  const workflow = page.locator(`.tab[data-tab-id="${workflowDocumentId}"]`)
  const imageDocumentId = await imageTab.getAttribute('data-tab-id')
  const videoDocumentId = await page
    .locator('.tab[data-tab-id^="video:"]')
    .getAttribute('data-tab-id')
  const imageBox = (await imageTab.boundingBox())!
  ...
  await page.mouse.move(imageBox.x + imageBox.width / 2 + 8, imageBox.y + 8)
  await page.mouse.move(workflowBox.x - 8, workflowBox.y + workflowBox.height / 2, {
    steps: 3,
  })
  await page.mouse.up()
  await expect
    .poll(() =>
      page
        .locator('.workflow-tablist [role="tab"]')
        .evaluateAll((tabs) =>
          tabs.map((tab) => tab.closest('.tab')?.getAttribute('data-tab-id')),
        ),
    )
    .toEqual([
      imageDocumentId,
  ...
  await page.screenshot({
    path: testInfo.outputPath('shared-document-tabs.png'),
    fullPage: true,
  })

  await page.reload()
  await expect(
    page.locator('.tab[data-tab-id^="video:"] .tab-select'),
  ).toHaveCount(1)
  await expect
    .poll(() =>
      page
  ...
  await page.locator('.tab[data-tab-id^="video:"] .tab-select').click()
  await expect(page.getByTestId('video-document-json')).toHaveValue(/1920/)
})

test('renders the current ImageDocument and changes mounted editor chrome locale', async ({
  page,
  request,
}) => {
  mkdirSync(proofDir(), { recursive: true })
  ...
  await page.route(/\/api\/assets(?:\/.*)?(?:\?.*)?$/, async (route) => {
    ...
    if (path.endsWith('/render') && request.method() === 'POST') {
      ...
      await route.fulfill({
        status: 201,
        json: { cacheKey, cached: false, asset, provenance },
      })
      return
    }
  ...
  await expect(
    workspace.getByRole('combobox', { name: 'Authoritative render target' }),
  ).toBeVisible()
  const authoritativeRenderCompleted = page.waitForResponse((response) => {
    const request = response.request()
    return (
      request.method() === 'POST' &&
      new URL(response.url()).pathname.endsWith('/render') &&
      response.status() === 201
    )
  })
  await Promise.all([
  ...
  await expect(
    result.getByRole('link', { name: 'Download PNG' }),
  ).toHaveAttribute('href', `/api/assets/${encodeURIComponent(outputDigest)}`)
  await expect(
    result.getByRole('img', { name: 'Authoritative ImageDocument output' }),
  ).toHaveJSProperty('naturalWidth', 900)
  await page.screenshot({
    path: `${localeProofDir()}/image-document-editor-i18n-en.png`,
    fullPage: true,
  })
  const requestsBeforeLocale = assetRequests
  const localeModule = await (await request.get('/src/locale.ts')).text()
  const i18nModule = localeModule.match(
  ...
  await expect(
    workspace.getByRole('combobox', { name: '[Referenz-Renderziel]' }),
  ).toContainText('[Vollstandiges Komposit]')
  await expect(workspace).toContainText('authoritative-source')
  await expect(result).toContainText(outputDigest)
  await expect(result).toContainText('dinkster-image-document-v2-cpu-reference')
  expect(assetRequests).toBe(requestsBeforeLocale)
  await page.screenshot({
    path: `${localeProofDir()}/image-document-editor-i18n-de-DE.png`,
    fullPage: true,
  })
  await workspace.screenshot({
  ...
  await workspace
    .getByRole('slider', { name: '[Ebenendeckkraft]' })
    .press('Home')
  await expect(workspace).toContainText('Stale - render again')
})

test('shares an image document and confirms ending it for everyone', async ({
  page,
  request,
}) => {
  mkdirSync(proofDir(), { recursive: true })
  ...
  await expect(
    workspace.getByRole('dialog', { name: 'End shared image session?' }),
  ).toBeVisible()
  await workspace.screenshot({
  ...
  await page.screenshot({
    path: `${localeProofDir()}/image-document-workspace-shell-en.png`,
    fullPage: true,
  })
  ...
  await expect(workspace.getByRole('tablist')).toHaveAttribute(
    'aria-label',
    '[Offene Bilddokumente]',
  )
  await expect(
    workspace.getByRole('button', { name: '[Freigabe beenden]', exact: true }),
  ).toBeVisible()
  await expect(workspace.locator('.image-document-shared-status')).toHaveText(
    '[Geteilt]',
  )
  await expect(
    workspace.getByRole('dialog', { name: '[Geteilte Bildsitzung beenden?]' }),
  ).toBeVisible()
  await expect(workspace.getByTestId('image-collab-end-confirm')).toHaveText(
    '[Geteilte Sitzung beenden]',
  )
  await expect(workspace.getByTestId('image-collab-end-cancel')).toHaveText(
    '[Abbrechen]',
  )
  await expect(workspace).toContainText('shared-canvas')
  expect(sessionRequests).toBe(requestsBeforeLocale)
  await page.screenshot({
    path: `${localeProofDir()}/image-document-workspace-shell-de-DE.png`,
    fullPage: true,
  })
})
