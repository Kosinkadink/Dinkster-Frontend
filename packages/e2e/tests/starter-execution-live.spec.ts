 ...
 *   DINKSTER_FRONTEND_REVISION, DINKSTER_TEST_HOST and DINKSTER_TEST_GPU.
 *   Each receipt records the execution timestamp, the exact harness
 *   invocation, revisions, model/output digests, the submitted graph, the
 *   terminal job, host/GPU identity, and the screenshot.
 ...
 * the reviewed starter rows before anything executes, over the FULL matrix
 * regardless of any family filter. Absent Qwen Image, TripoSplat, Wan 2.1
 * or Wan 2.2 templates are the composition blocker (wrapper issue 248:
 * those packs do not compose in a normal launch): they are reported to the
 * caller and to every executing receipt but never prevent the available
 * non-blocked rows from executing and producing receipts. Every other
 * gap, duplicate, id drift, or unlisted dinkster.* family is an ordinary
 * failure. No skipping, reloading, or pack retries.
 */
...
/** Execution timestamp from the terminal job's backend-recorded submission time. */
...
const HARNESS_COMMAND =
  'pnpm --filter @dinkster/e2e exec playwright test tests/starter-execution-live.spec.ts --project=backend-serial --workers=1'

/** Exact harness invocation: canonical command, worker process, env contract. */
function invocationRecord(): Record<string, unknown> {
  const executionFlag = process.env['DINKSTER_STARTER_EXECUTION_E2E']
  const overridesPath = process.env['DINKSTER_STARTER_OVERRIDES']
  return {
    ...
    environment: {
      ...
      ...(executionFlag === undefined ? {} : { DINKSTER_STARTER_EXECUTION_E2E: executionFlag }),
      ...(FAMILY_FILTER === undefined ? {} : { DINKSTER_STARTER_EXECUTION_FAMILY: FAMILY_FILTER }),
      ...(overridesPath === undefined ? {} : { DINKSTER_STARTER_OVERRIDES: overridesPath }),
      ...(RECEIPT_DIR === undefined ? {} : { DINKSTER_STARTER_RECEIPT_DIR: RECEIPT_DIR }),
    },
  }
}
...
for (const row of STARTER_ROWS) {
  test(`executes the ${row.templateId} starter end to end through the real backend`, async ({
    ...
    if (entry === undefined) {
      // Only an absent issue-248 composition-blocker family reaches here:
      // every other catalog gap already failed the full-matrix preflight.
