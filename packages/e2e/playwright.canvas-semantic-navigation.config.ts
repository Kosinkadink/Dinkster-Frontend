...
const frontendPort = port('DINKSTER_CANVAS_SEMANTIC_FRONTEND_PORT', 5739)
const fixturePort = port('DINKSTER_CANVAS_SEMANTIC_FIXTURE_PORT', 5749)
if (frontendPort === fixturePort) throw new Error('canvas semantic frontend and fixture ports must differ')
const repoRoot = process.env['DINKSTER_CANVAS_SEMANTIC_REPO_ROOT'] ?? '../..'

export default defineConfig({
  testDir: './tests',
  testMatch: 'canvas-semantic-navigation.spec.ts',
  timeout: 60_000,
  workers: 1,
  reporter: 'line',
  projects: [
