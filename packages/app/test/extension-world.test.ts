...
import {
  ExtensionWorld,
  type FrontendActivationContext,
} from '../src/extension-world.js'
...
function setup() {
  ...
  const target: ExtensionHostOptions<TextWidgetEditorExtension> = {
    ...
    registerEditor: (value) => {
      editors.push(value)
      return () => {
        editors.splice(editors.indexOf(value), 1)
      }
    },
    registerEditorBinding: (value) => {
      editorBindings.push(value)
      return () => {
        editorBindings.splice(editorBindings.indexOf(value), 1)
      }
    },
    registerPanel: (value) => {
      panels.push(value)
      return () => {
        panels.splice(panels.indexOf(value), 1)
      }
    },
    registerVirtualNode: (value) => {
      virtualNodes.push(value)
      return () => {
        virtualNodes.splice(virtualNodes.indexOf(value), 1)
      }
    },
  }
...
function snapshotFor(
  ...
  privilege: FrontendPrivilege,
): EffectiveExtensionSnapshot {
  return {
    ...
    extensions: [
      {
        ...
        frontend: [
          {
            id: 'demo.entry',
            moduleUrl: `/api/extension-assets/demo/${digest}/demo.entry.js`,
            moduleDigest: digest,
            ...
            contributions: [
              {
                ...
                kind,
                ...(kind === 'eventConsumer' ? { event: 'demo.event' } : {}),
              },
            ],
          },
        ],
...
const extensionEvent = (seq: number): ExtensionEvent => ({
  kind: 'extensionEvent',
  execution: { connection: asConnectionId('a'), prompt: asPromptId('job') },
  timestamp: 0,
  extensionSnapshotDigest: digest,
  event: 'demo.event',
  pack: 'demo',
  schemaVersion: 1,
  data: { value: seq },
...
describe('connection extension worlds', () => {
  it('projects a virtual node through the selected pack world and removes it on disposal', async () => {
    ...
    const kind: VirtualNodeKind = {
      ...
      render: (node) => ({
        text: String(node.values['text'] ?? ''),
        format: 'plain',
      }),
    }
    await world.activate(
      snapshotFor('virtualNode', 'graph-editor-canvas'),
      '',
      [],
      async () => ({
        frontendExtension: {
          activate(context: FrontendActivationContext) {
  ...
  it('rejects a virtual node kind that is already registered', () => {
    ...
    try {
      ...
      expect(() => register(kind)).toThrow(
        "virtual node kind 'dinkster.note' is already registered",
      )
  ...
  it('preserves editor role lookup when virtual node schemas are included', () => {
    vi.stubGlobal('location', { protocol: 'http:', host: 'test' })
    const app = new AppState()
    try {
      const schema: NodeSchema = {
        type: 'demo.editor',
        editorRole: 'demo-role',
        displayName: 'Editor',
        category: 'Demo',
        source: 'v3',
        isOutputNode: false,
      ...
      const fallback: SchemaResolver = Object.assign(() => undefined, {
        forEditorRole: (role: string) =>
          role === 'demo-role' ? schema : undefined,
      })
      const resolve = (
        app as unknown as {
          resolverWithVirtualNodes: (
            candidate: SchemaResolver,
          ) => SchemaResolver
        }
      ).resolverWithVirtualNodes(fallback)
      expect(resolve.forEditorRole?.('demo-role')).toBe(schema)
    } finally {
      app.dispose()
      vi.unstubAllGlobals()
    }
  ...
  it.each([
    ['widgetKind', 'schema-widget'],
    ['menu', 'graph-editor-canvas'],
    ['virtualNode', 'graph-editor-canvas'],
    ['setting', 'app-workflow'],
    ['eventConsumer', 'event-consumer'],
  ] as const)(
    'restricts %s before importing code without widening another privilege',
    async (kind, privilege) => {
      const { target } = setup()
      const world = new ExtensionWorld(asConnectionId('a'), digest, target)
      const load = vi.fn()
      ...
      const unauthorized = new ExtensionWorld(
        asConnectionId('b'),
        digest,
        target,
      )
      await unauthorized.activate(
        snapshotFor(
          kind,
          privilege === 'event-consumer' ? 'app-workflow' : 'event-consumer',
        ),
        '',
        [],
        load,
  ...
  it.each(['workflowObserver', 'futureKind'])(
    'skips an unimplemented or unknown %s contribution while activating its sibling',
    async (kind) => {
      ...
      await world.activate(
              ...
              frontend: [
                {
                  ...
                  contributions: [
                    ...entry.contributions,
                    { id: 'demo.skipped', kind },
                  ] as typeof entry.contributions,
                },
              ],
            },
          ],
        ...
        async () => ({
          frontendExtension: {
            activate(context: FrontendActivationContext) {
              context.command('demo.contribution', {
                id: 'demo.contribution',
                label: 'Active',
                run() {},
              })
      ...
      expect(
        world.host.packs()[0]?.contributions.map((item) => item.decl.id),
      ).toEqual(['demo.contribution'])
      expect(report).toHaveBeenCalledWith(
        expect.objectContaining({
          severity: 'warning',
          code: 'extension.contribution-kind-skipped',
  ...
  it('delivers only declared producer events asynchronously and cancels queued delivery on gate changes', async () => {
    ...
    await world.activate(
      snapshotFor('eventConsumer', 'event-consumer'),
      '',
      [],
      async () => ({
        frontendExtension: {
          activate(context: FrontendActivationContext) {
            ...
            context.eventConsumer('demo.contribution', (event) =>
              received.push(event),
            )
          },
        },
    ...
    world.deliver({
      ...extensionEvent(0),
      execution: { connection: asConnectionId('b'), prompt: asPromptId('job') },
    })
    ...
    expect(received.map((event) => event.seq)).toEqual([
      1,
      ...Array.from({ length: 64 }, (_, index) => index + 39),
    ])
    world.dispose()
  ...
  it('rolls back a caught scope violation instead of admitting sibling registrations', async () => {
    ...
    expect(world.diagnostics.map((diagnostic) => diagnostic.code)).toEqual([
  ...
  it.each(['GET', 'POST'] as const)(
    'confines %s queries to typed own-pack snapshot routes and tears them down',
    async (method) => {
      ...
      Object.defineProperty(globalThis, 'location', {
        configurable: true,
        writable: true,
        value: { href: 'http://test/', origin: 'http://test' },
      })
      const fetch = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
        new Response('{"count":4}', {
          headers: {
            'Content-Type': 'application/json',
            'X-Dinkster-Extension-Snapshot': digest,
          },
        }),
      )
      ...
      try {
        await world.activate(
                ...
                routes: [
                  {
                    ...
                    method,
                    handler: 'demo:policy',
                    request: method === 'POST' ? { count: 'integer' } : {},
          ...
          async () => ({
            frontendExtension: {
              activate(api: FrontendActivationContext) {
                context = api
                api.hostUi('demo.contribution', 'status.trailing', () => ({
                  version: 1,
                  root: { kind: 'text', key: 'text', text: 'Preview' },
                }))
              },
            },
          }),
        ...
        expect(() => context.invalidateHostUi('other.contribution')).toThrow(
          'cannot invalidate',
        )
        await expect(
          context.queryRoute('other.policy', { count: 4 }),
        ).rejects.toThrow('undeclared own-pack')
        await expect(
          context.queryRoute('policy', { count: 4.5 }),
        ).rejects.toThrow('undeclared own-pack')
        ...
        expect(String(url)).toBe(
          'http://test/a/api/extensions/demo/routes/policy',
        )
        ...
        fetch.mockResolvedValueOnce(
          new Response('{"count":4}', {
            headers: {
              'Content-Type': 'application/json',
              'X-Dinkster-Extension-Snapshot': 'other',
            },
          }),
        )
        await expect(context.queryRoute('policy', input)).rejects.toThrow(
          'unpaired response',
        )
        fetch.mockResolvedValueOnce(
          new Response('{"count":"bad"}', {
            headers: {
              'Content-Type': 'application/json',
              'X-Dinkster-Extension-Snapshot': digest,
            },
  ...
  it('never grants a route query or UI invalidation to an event-only consumer', async () => {
    ...
    await expect(context.queryRoute('policy')).rejects.toThrow(
      'app-workflow authority',
    )
    expect(() => context.invalidateHostUi('demo.contribution')).toThrow(
      'cannot invalidate',
    )
    world.dispose()
  })
  ...
  it('isolates a throwing consumer without blocking sibling queues', async () => {
    ...
    expect(world.diagnostics.map((item) => item.code)).toEqual([
      'extension.consumer-failed',
    ])
    world.dispose()
  })
      ...
      for (const backend of [a, b]) {
        ...
        expect(
          app.extensions.register(
            {
              id: 'demo',
              contributions: [{ id: 'demo.command', category: 'command' }],
            },
            (api) => {
              api.command('demo.command', {
                id: 'demo.command',
                label: backend.label,
                run: () => {},
              })
  ...
  it('retains the compiled world for a frozen run and disposes it after eviction', async () => {
    ...
    try {
      ...
      const nodes = JSON.parse(
        readFileSync(
          new URL(
            '../../core/fixtures/dinkster-nodes-comfy.json',
            import.meta.url,
          ),
          'utf8',
        ),
      ...
      const snapshot: EffectiveExtensionSnapshot = {
        format: 'dinkster.extension-snapshot',
        version: 1,
        frontendApi: '1.0.0',
        extensions: [],
      ...
      fetch.mockResolvedValue({
        ...registry,
        extensionSnapshotPair: { digest, snapshot },
      })
      await app.refreshBackendSchemas(backend)
      app.setTabTarget(tab.id, backend.id)
      ...
      fetch.mockResolvedValue({
        ...registry,
        extensionSnapshotPair: { digest: `sha256:${'b'.repeat(64)}`, snapshot },
      })
      await app.refreshBackendSchemas(backend)
  ...
  it('rolls back failed activation and aborts/disposes only its own instance', () => {
    ...
    const calls: string[] = []
    const diagnostics = world.host.register(
      {
        id: 'demo',
        contributions: [{ id: 'demo.command', category: 'command' }],
      },
  ...
  it('projects API 1.1 editor doors only while the snapshot world is selected', async () => {
      ...
      async () => ({
        frontendExtension: {
          activate(api: FrontendActivationContext) {
            ...
            api.editor('demo.editor', {
              id: 'demo.editor',
              title: 'Demo editor',
              provider,
            })
            api.editorBinding('demo.binding', {
              id: 'demo.binding',
              editor: 'demo.editor',
              match: { editorRole: 'demo' },
            })
    ...
    expect(
      [editors, editorBindings, panels].map((values) => values.length),
    ).toEqual([0, 0, 0])
    world.select(true)
    expect(
      [editors, editorBindings, panels].map((values) => values.length),
...
describe('submission-owned extension world lifetime', () => {
  ...
  const refresh = async (letter: string) => {
    ...
    vi.spyOn(backend.connection, 'fetchSchemas').mockResolvedValue({
      ...buildDinksterRegistry(backend.id, nodes),
      extensionSnapshotPair: {
  ...
  const accepted = (
    job = 'held',
  ): Extract<DinksterSubmitResult, { ok: true }> => ({
    ok: true,
    execution: { connection: backend.id, prompt: asPromptId(job) },
  })
  const missing = (
    retryWithAssets: Extract<
  ...
  const expectFrozenWorld = (job = 'held') => {
    ...
    expect(app.registryForExecution(ref)?.extensionSnapshotPair?.digest).toBe(
      digest,
    )
    expect(app.openExecutionView(ref)).toBe(true)
    expect(app.extensions).toBe(oldHost)
  ...
  beforeEach(async () => {
    ...
    if (!added || added.protocol !== 'dinkster')
      throw new Error('expected native backend')
    backend = added
    vi.spyOn(backend, 'refreshDiagnostics').mockImplementation(() => {})
    vi.spyOn(backend.connection, 'uploadAsset').mockResolvedValue(
      'blake3:source',
    )
    await refresh('a')
    tab = app.tabs.get()[0]!
  ...
  it.each(['source upload', 'submit response'])(
    'retains compile-time world across a held %s and transfers it to the frozen run',
    async (phase) => {
      const upload = deferred<string>()
      const response = deferred<DinksterSubmitResult>()
      if (phase === 'source upload')
        vi.spyOn(backend.connection, 'uploadAsset').mockReturnValue(
          upload.promise,
        )
      const submit = vi
        .spyOn(backend.connection, 'submit')
        .mockReturnValue(response.promise)
      const queued = app.queue(tab)
      if (phase === 'submit response')
  ...
  it.each(['rejection', 'transport failure', 'closed before POST'])(
    'releases a held submission on %s',
    async (outcome) => {
      const upload = deferred<string>()
      const response = deferred<DinksterSubmitResult>()
      ...
      if (outcome === 'closed before POST') {
        ...
        response.resolve({ ok: false, diagnostics: [] })
  ...
  it.each(['cancel', 'close tab', 'replace tab', 'deny', 'dispose'])(
    'releases idle consent ownership on %s after queue returns',
    async (action) => {
      const retry = vi.fn(
        async (): Promise<DinksterSubmitResult> => ({
          ok: false,
          diagnostics: [],
        }),
      )
      ...
      else if (action === 'replace tab')
        app.openDocument(tab.store.doc, 'Replacement')
  ...
  it('resolves pane-owned kinds, validators, views, previews and core fallbacks instead of the shell registry', async () => {
        ...
        host.register(
          {
            id: 'widgets',
            contributions: [
              { id: 'widgets.kind', category: 'widgetKind' },
              { id: 'widgets.view', category: 'widgetView' },
          ...
          (api) => {
            api.widgetKind('widgets.kind', {
              ...
              defaultView: () => 'widgets.view',
              validate: () => [
                {
                  severity: 'warning',
                  origin: 'schema',
                  code: 'widgets.version',
                  message: String(version),
                },
              ],
            })
            api.widgetView('widgets.view', {
              id: 'widgets.view',
              kind: 'widgets.kind',
              isCompatible: () => true,
              measure: () => ({ rows: version }),
              drawCompact() {},
            })
            api.previewRenderer('widgets.preview', {
              id: 'widgets.preview',
              mediaKind: version === 1 ? 'image' : 'video',
              canRender: (channel) => channel === 'widgets/preview',
              drawCompact() {},
            })
          },
        ),
    ...
    for (const focused of [frozen, tab]) {
      ...
      for (const [pane, version] of [
        ...
        expect(
          registry.kind(spec.widgetType)?.validate(0, spec)[0]?.message,
        ).toBe(String(version))
        expect(registry.viewsFor(spec.widgetType)[0]?.measure(spec).rows).toBe(
          version,
        )
        expect(registry.previewRendererFor('widgets/preview')?.mediaKind).toBe(
          version === 1 ? 'image' : 'video',
        )
