...
type Note = { readonly title: string; readonly count: number; readonly editable: boolean }
const note: Note = { title: 'initial', count: 0, editable: true }
const problem = (message: string): readonly Diagnostic[] => [{
  severity: 'error', origin: 'command', code: 'note.invalid', message,
}]
...
const noteAdapter: DocumentTypeAdapter<Note> = {
  ...
  check(document) {
    return document !== null && typeof document.title === 'string' &&
      Number.isSafeInteger(document.count) && document.count >= 0 && typeof document.editable === 'boolean'
      ? [] : problem('invalid note')
  },
  execute(document, invocation) {
    if (!document.editable) return { ok: false, diagnostics: problem('note was locked by another actor') }
    const forward: PatchOp[] = invocation.command === 'note.increment'
      ? [{ op: 'replace', path: ['count'], value: document.count + 1, oldValue: document.count }]
      : [{ op: 'replace', path: ['title'], value: invocation.params, oldValue: document.title }]
    return {
      ok: true,
...
class Connection implements CollabConnection {
  ...
  revision = 0
  document: unknown
  post: (operation: CollabClientOp) => Promise<PostOpOutcome> = async (operation) => this.accept(operation)
  fetchOps: (after: number) => Promise<FetchOpsOutcome> = async (after) => ({
    kind: 'ops', ops: this.log.filter((operation) => operation.revision > after),
  })
  ...
  accept(operation: CollabClientOp): PostOpOutcome {
    if (operation.baseRevision !== this.revision) return { kind: 'stale-base', revision: this.revision }
    this.document = applyOps(this.document as Json, operation.patch)
    const op: CollabServerOp = { ...operation, revision: ++this.revision, timestamp: this.revision }
    this.log.push(op)
  ...
  async fetchSnapshot() { return this.snapshot }
  async putSnapshot(revision: number, document: unknown) {
    this.snapshots.push({ revision, document })
    return { kind: 'ok' as const }
  }
  sendPresence(payload: Json): void { this.presence.push(payload) }
  onEvent(listener: (event: CollabConnectionEvent) => void): () => void {
    this.listener = listener
    return () => { this.listener = undefined }
  }
  emit(event: CollabConnectionEvent): void { this.listener?.(event) }
  connected(kind = noteAdapter.kind): void {
    this.emit({ kind: 'connected', descriptor: {
      protocolVersion: COLLAB_PROTOCOL_VERSION, sessionId: this.sessionId,
      scope: 'shared', documentId: 'document', documentKind: kind,
      revision: this.revision, snapshotRevision: this.snapshot.revision,
    } })
  }
  foreign(patch: CollabClientOp['patch'], deliver = true): void {
    const result = this.accept({ protocolVersion: 1, actorId: 'peer', opId: `peer-${this.revision}`, baseRevision: this.revision, patch })
...
async function open(connection = new Connection(), adapter = noteAdapter, conflicts: SessionConflict[] = []) {
  const session = await connectDocumentSession(connection, adapter, {
    actorId: 'local', onConflict: (conflict) => conflicts.push(conflict),
    onListenerError: () => {}, snapshotRandom: () => 0,
  })
...
describe('document type registry', () => {
  it('normalizes legacy builtins but leaves extension kinds alone', () => {
    ...
    expect(() => registry.register({ ...noteAdapter, kind: 'workflow' })).toThrow('already registered')
    const descriptor = { protocolVersion: 1, sessionId: 'test', revision: 0, snapshotRevision: 0 }
    expect(validateCollabDescriptor(descriptor, 'test', 'dinkster.workflow')).toBeNull()
    expect(validateCollabDescriptor({ ...descriptor, documentKind: 'image' }, 'test', 'dinkster.image')).toBeNull()
    expect(validateCollabDescriptor({ ...descriptor, documentKind: 'dinkster.image' }, 'test', 'image')).toBeNull()
    expect(validateCollabDescriptor({ ...descriptor, documentKind: 'example.note' }, 'test', 'example.note')).toBeNull()
    expect(validateCollabDescriptor(descriptor, 'test', 'example.note')).toContain('mismatch')
  })

  it('refuses an invalid local actor identity before opening a transport session', () => {
    const connection = new Connection()
    expect(() => new SharedDocumentSession(connection, note, 0, noteAdapter, { actorId: '__proto__' }))
...
describe('third document adapter through the shared engine', () => {
  it('dispatches, rebases a foreign edit, and sends undo/redo through the same FIFO', async () => {
    ...
    await session.settle()
    expect(session.doc.count).toBe(5)
    expect(operations).toEqual(['remote', 'note.increment', 'session.undo', 'session.redo'])
    expect(connection.posts.map((operation) => operation.baseRevision)).toEqual([1, 2, 3])
    ...
    connection.emit({ kind: 'presence', actorId: 'peer', payload: { cursor: 4 } })
  ...
  it('catches up a gap and resyncs without discarding an unsent intention', async () => {
    ...
    connection.fetchOps = async () => ({ kind: 'resync-required', snapshotRevision: 2 })
  ...
  it('reports rebase and history conflicts as diagnostics for Problems, never overwriting a peer', async () => {
    ...
    await session.settle()
    connection.foreign([{ op: 'replace', path: ['title'], value: 'peer title' }])
    ...
    expect(conflicts.map((conflict) => conflict.during)).toEqual(['undo', 'rebase'])
    expect(conflicts.every((conflict) => conflict.diagnostics[0]?.severity === 'error')).toBe(true)
    expect(connection.posts).toHaveLength(1)
  })
  ...
  it('publishes timed checkpoints from confirmed state while an optimistic edit is pending', async () => {
    ...
    connection.post = () => new Promise((done) => { resolve = done })
    session.dispatch({ command: 'note.increment', params: null })
    await vi.advanceTimersByTimeAsync(60_000)
    expect(session.doc.count).toBe(21)
    expect(connection.snapshots).toEqual([{ revision: 20, document: { ...note, count: 20 } }])
  ...
  it('keeps definitive security denials terminal and rejects settle without retry', async () => {
    const connection = new Connection()
    const diagnostic = { version: 1 as const, type: 'collab.denial' as const, code: 'forbidden', status: 403, message: 'denied', sessionId: connection.sessionId, actorId: 'local', opId: 'denied-op' }
  ...
  it('owns adapter patches and derives state from them rather than retaining proposed mutable state', async () => {
    const connection = new Connection()
    const patch: PatchOp[] = [{ op: 'replace', path: ['title'], value: 'owned', oldValue: 'initial' }]
    const proposed = { ...note, title: 'not the patch' }
    const session = await open(connection, { ...noteAdapter, execute: () => ({ ok: true, doc: proposed, forward: patch, inverse: invertOps(patch), diagnostics: [] }) })
    expect(session.dispatch({ command: 'note.title', params: 'owned' }).ok).toBe(true)
    patch.length = 0
  ...
  it.each([
    ...
    [{ op: 'replace', path: ['__proto__', 'polluted'], value: true, oldValue: null }],
    ...
    const session = await open(connection, { ...noteAdapter, execute: () => ({ ok: true, doc: note, forward: patch, inverse: [], diagnostics: [] }) })
    expect(session.dispatch({ command: 'note.title', params: null }).ok).toBe(false)
  ...
  it('reserves session.patch even if the adapter advertises it and rejects malformed preparation', async () => {
    ...
    const session = await open(connection, {
      ...
      prepare: () => ({ ok: true, params: { invalid: undefined } as unknown as Json }),
    })
    expect(session.dispatch({ command: 'session.patch', params: { ops: [] } })).toMatchObject({ ok: false, diagnostics: [{ code: 'command.reserved' }] })
    expect(session.dispatch({ command: 'note.title', params: 'bad' })).toMatchObject({ ok: false, diagnostics: [{ code: 'command.params.notJson' }] })
    expect(connection.posts).toHaveLength(0)
  })
  ...
  it('isolates a malformed transform as a conflict without losing other pending intentions', async () => {
    ...
    const session = await open(connection, {
      ...noteAdapter,
      transform: (invocation) => invocation.command === 'note.title' ? { invalid: undefined } as unknown as Json : undefined,
    }, conflicts)
  ...
  it('rejects non-JSON loader output before subscribing', async () => {
    ...
    await expect(open(connection, { ...noteAdapter, load: () => ({ document: { ...note, extra: () => {} }, diagnostics: [] }) })).rejects.toThrow('not JSON')
  ...
  it('carries resource retention with adopted history and releases it on clear', async () => {
    const adapter: DocumentTypeAdapter<Note> = {
      ...noteAdapter, resourceDigests: (document) => new Set([document.title]),
    }
    ...
    expect(second.retainedResourceDigests()).toEqual(new Set(['initial', 'resource']))
...
const workflow: WorkflowDocument = {
  format: 'dinkster-workflow', formatVersion: 1, lineage: asLineageId('lineage'), root: asGraphDefId('g0'),
  graphs: { g0: { id: asGraphDefId('g0'), name: 'graph', nodes: { n0: { id: asNodeId('n0'), type: 'X', values: {} } }, links: {}, nets: {}, reroutes: {}, nextOrdinal: 1 } },
  view: { graphs: {} },
}
const image: ImageDocument = {
  format: 'dinkster-image', formatVersion: 2, lineage: asImageLineageId('lineage'),
  canvas: { width: 1, height: 1, colorSpace: 'srgb', channelDepth: 8, compositing: 'premultiplied-alpha' },
  allocation: { nextOrdinal: 1 }, rootLayerIds: [asImageLayerId('l0')],
  layers: { l0: { id: asImageLayerId('l0'), kind: 'group', name: 'initial', childLayerIds: [], visible: true, opacity: 65535, transform: { a: 1000000, b: 0, c: 0, d: 1000000, tx: 0, ty: 0 }, blendMode: 'normal', clipping: 'none', maskIds: [] } },
  masks: {}, resources: {},
}

describe('transport denials across document adapters', () => {
  it.each(['workflow', 'image', 'example.note'])('fails an idle %s session immediately without losing the document', async (kind) => {
    const initial = kind === 'workflow' ? workflow : kind === 'image' ? image : note
    const connection = new Connection(initial)
    const errors: string[] = []
    const options = { actorId: 'local', onError: (message: string) => errors.push(message) }
    const session = kind === 'workflow'
      ? await connectSharedSession(connection, coreCommandRegistry(), options)
      : kind === 'image'
        ? new SharedImageDocumentSession(connection, image, 0, options)
        : await connectDocumentSession(connection, noteAdapter, options)
    sessions.push(session)
    const diagnostic = {
      version: 1 as const, type: 'collab.denial' as const, code: 'forbidden', status: 403,
      message: 'session access revoked', sessionId: connection.sessionId, actorId: 'local', operation: 'snapshot',
    }
...
describe('default positional array history', () => {
  const adapter: DocumentTypeAdapter<{ items: string[] }> = {
    kind: 'example.list', commandIds: new Set(['remove', 'insert']),
    load: (value) => ({ document: value as { items: string[] }, diagnostics: [] }),
    check: () => [],
    execute(document, invocation) {
      const forward: PatchOp[] = invocation.command === 'remove'
        ? [{ op: 'remove', path: ['items', 0], oldValue: document.items[0]! }]
        : [{ op: 'add', path: ['items', 0], value: 'new' }]
      return { ok: true, doc: applyOps(document, forward) as { items: string[] }, forward, inverse: invertOps(forward), diagnostics: [] }
    },
  }

  it.each(['remove', 'insert'])('undoes and redoes a non-tail %s without a foreign edit', async (command) => {
    const initial = { items: ['a', 'b'] }
    const connection = new Connection(initial)
    const conflicts: SessionConflict[] = []
    const session = await connectDocumentSession(connection, adapter, { actorId: 'local', onConflict: (conflict) => conflicts.push(conflict) })
    sessions.push(session)
...
describe('definitively refused stale-base attempts', () => {
  it('keeps an earlier unknown attempt ambiguous when a reminted attempt is refused', async () => {
    ...
    const conflicts: SessionConflict[] = []
    const session = await open(connection, noteAdapter, conflicts)
    connection.fetchOps = async () => ({ kind: 'resync-required', snapshotRevision: 1 })
    connection.post = async () => {
      if (connection.posts.length === 1) {
        connection.emit({ kind: 'op', op: { opId: 'invalid' } as CollabServerOp })
    ...
    expect(session.doc.count).toBe(1)
    expect(conflicts).toHaveLength(1)
    expect(conflicts[0]?.diagnostics[0]?.code).toBe('session.delivery-ambiguous')
  })

  it.each(['workflow', 'image', 'example.note'])('retains the %s edit across a compacted resync and submits exactly twice', async (kind) => {
    const initial = kind === 'workflow' ? workflow : kind === 'image' ? image : note
    const connection = new Connection(initial)
    connection.revision = 1
    ...
    const options = { actorId: 'local', onConflict: (conflict: unknown) => conflicts.push(conflict) }
    if (kind === 'workflow') {
      const session = await connectSharedSession(connection, coreCommandRegistry(), options)
      sessions.push(session)
      session.dispatch({ command: 'node.setTitle', params: { graphId: 'g0', nodeId: 'n0', title: 'retained' } })
      await session.settle()
      expect(session.doc.graphs.g0?.nodes.n0?.title).toBe('retained')
    } else if (kind === 'image') {
      const session = new SharedImageDocumentSession(connection, image, 0, options)
      sessions.push(session)
      session.dispatch({ command: 'image.layer.update', params: { layerId: 'l0', name: 'retained' } })
      await session.settle()
      expect(session.doc.layers.l0?.name).toBe('retained')
    } else {
      const session = await open(connection, noteAdapter, conflicts as SessionConflict[])
      session.dispatch({ command: 'note.title', params: 'retained' })
      await session.settle()
      expect(session.doc.title).toBe('retained')
    }
    expect(connection.posts).toHaveLength(2)
    expect(connection.posts.map((operation) => operation.baseRevision)).toEqual([0, 1])
...
describe('image adapter history and resource retention', () => {
  it('keeps allocation writes out of redo and retains resources through rebased history', async () => {
    ...
    sessions.push(session)
    const result = session.dispatch({ command: 'image.layer.addRaster', params: {
      parentId: null, index: 1, name: 'raster',
      resource: { kind: 'raster', digest, byteSize: 4, mediaType: 'image/png', width: 1, height: 1, colorSpace: 'srgb', channelDepth: 8, alphaMode: 'straight' },
      sourceRect: { x: 0, y: 0, width: 1, height: 1 },
    } })
    expect(result).toMatchObject({ ok: true, created: { resourceId: 'r0-local', layerId: 'l1-local' } })
    connection.foreign([{ op: 'replace', path: ['layers', 'l0', 'name'], value: 'peer group' }])
    await session.settle()
    expect(session.doc.allocation.actorCursors?.local).toBe(2)
    expect(session.undo()).toBe(true)
    connection.foreign([{ op: 'replace', path: ['layers', 'l0', 'name'], value: 'peer renamed' }])
    await session.settle()
    expect(session.doc.resources).toEqual({})
    expect(session.retainedResourceDigests()).toEqual(new Set([digest]))
    expect(session.doc.allocation.actorCursors?.local).toBe(2)
    expect(session.redo()).toBe(true)
    connection.foreign([{ op: 'replace', path: ['layers', 'l0', 'name'], value: 'peer again' }])
    await session.settle()
    expect(session.doc.layers['l1-local']).toBeDefined()
    expect(connection.posts.at(-1)?.patch.every((operation) => operation.path[0] !== 'allocation')).toBe(true)
    expect(session.undo()).toBe(true)
  ...
  it('rejects a foreign patch that violates image shape even if references remain valid', async () => {
    const connection = new Connection(image)
    const session = await connectDocumentSession(connection, imageDocumentTypeAdapter, {
      actorId: 'local', onListenerError: () => {},
    })
    sessions.push(session)
    connection.emit({ kind: 'op', op: {
      opId: 'invalid-shape', actorId: 'peer', baseRevision: 0, revision: 1, timestamp: 1,
      patch: [{ op: 'replace', path: ['canvas', 'width'], value: -1 }],
    } })
    // The authoritative snapshot and retained log contain a valid peer edit.
    connection.foreign([{ op: 'replace', path: ['layers', 'l0', 'name'], value: 'valid peer' }], false)
    await session.settle()
