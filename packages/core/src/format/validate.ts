...
function checkTransform(checker: ImageShapeChecker, value: unknown, path: string, version: number): void {
...
function checkSourceRect(checker: ImageShapeChecker, value: unknown, path: string): void {
  if (!checker.object(value, path)) return
...
function checkLayer(checker: ImageShapeChecker, value: unknown, path: string, version: number): void {
  if (!checker.object(value, path)) return
...
function checkMask(checker: ImageShapeChecker, value: unknown, path: string, version: number): void {
  if (!checker.object(value, path)) return
...
function checkResource(checker: ImageShapeChecker, value: unknown, path: string, version: number): void {
  if (!checker.object(value, path)) return
  if (value['kind'] !== 'raster') {
...
function checkRecord(
  ...
  path: string,
  maximum: number,
  checkEntry: (checker: ImageShapeChecker, value: unknown, path: string) => void,
): void {
  if (!checker.object(value, path)) return
...
export function validateImageDocumentShape(
  ...
  const checker = new ImageShapeChecker()
