...
function imageError(code: string, message: string): Diagnostic {
...
function checkMapIdentity(
  ...
  for (const [key, value] of Object.entries(record)) {
    if (value.id !== key) {
      diagnostics.push(imageError('image.id.keyMismatch', `${kind} '${key}' stores id '${value.id}'`))
    }
  }
}
...
function checkLayerTree(document: ImageDocument, diagnostics: Diagnostic[]): void {
  ...
  const countParent = (id: string, owner: string): void => {
    if (getOwn(document.layers, id) === undefined) {
      diagnostics.push(imageError('image.layer.dangling', `${owner} references missing layer '${id}'`))
      return
    }
  ...
  const countList = (ids: readonly string[], owner: string): void => {
    ...
    ids.forEach((id, index) => {
      if (seen.has(id)) {
        diagnostics.push(imageError('image.layer.duplicateChild', `${owner} repeats layer '${id}' at index ${index}`))
      }
      seen.add(id)
    ...
    if (first !== undefined && getOwn(document.layers, first)?.clipping === 'clip-to-previous') {
      diagnostics.push(imageError('image.layer.clippingTarget', `first layer '${first}' in ${owner} has no previous sibling to clip to`))
    }
  }
  ...
  for (const id of Object.keys(document.layers)) {
    ...
    if (count !== 1) {
      diagnostics.push(imageError('image.layer.parentCount', `layer '${id}' appears in ${count} parent lists; expected exactly one`))
    }
  }
  ...
  const visit = (id: string, depth: number, fromRoot: boolean): void => {
    ...
    if (state.get(id) === 'visiting') {
      diagnostics.push(imageError('image.layer.cycle', `layer tree contains a cycle at '${id}'`))
      return
    }
  ...
  for (const id of Object.keys(document.layers)) {
    if (!rooted.has(id)) {
      diagnostics.push(imageError('image.layer.unreachable', `layer '${id}' is not reachable from rootLayerIds`))
    }
...
function checkResourceReference(
  ...
  if (resource === undefined) {
    diagnostics.push(imageError('image.resource.dangling', `${owner} references missing resource '${resourceId}'`))
...
function checkMasksAndResources(document: ImageDocument, diagnostics: Diagnostic[]): void {
  ...
  for (const layer of Object.values(document.layers)) {
    ...
    for (const maskId of layer.maskIds) {
      if (seen.has(maskId)) {
        diagnostics.push(imageError('image.mask.duplicate', `layer '${layer.id}' repeats mask '${maskId}'`))
      }
      seen.add(maskId)
      const mask = getOwn(document.masks, maskId)
      if (mask === undefined) {
        diagnostics.push(imageError('image.mask.dangling', `layer '${layer.id}' references missing mask '${maskId}'`))
        continue
      }
  ...
  for (const mask of Object.values(document.masks)) {
    ...
    if (count !== 1) {
      diagnostics.push(imageError('image.mask.ownerCount', `mask '${mask.id}' appears in ${count} layer mask lists; expected exactly one`))
    }
    if (getOwn(document.layers, mask.ownerLayerId) === undefined) {
      diagnostics.push(imageError('image.mask.danglingOwner', `mask '${mask.id}' names missing owner layer '${mask.ownerLayerId}'`))
    }
